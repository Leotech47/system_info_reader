...
def main():
    """Função principal"""
    parser = argparse.ArgumentParser(description='Coletor de Informações do Sistema')
    parser.add_argument('--save', '-s', metavar='FILENAME',
                        help='Salvar informações em arquivo JSON e sair')
    parser.add_argument('--summary', action='store_true',
                        help='Mostrar apenas resumo do sistema e sair')
    parser.add_argument('--quiet', '-q', action='store_true',
                        help='Modo silencioso (apenas para salvar arquivo)')
    ...
    if args.save:
        try:
            reader.save_to_file(args.save)
            if not args.quiet:
                print(f"Informações salvas em {args.save}")
        except Exception as e:
            if not args.quiet:
    ...
    if args.summary:
        try:
            ...
            print(f"Processos em execução: {len(info.get('processes', []))}")
        except Exception as e:
